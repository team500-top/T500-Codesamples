...
$(function(){
    ...
    cG.showTotalHashrate = $.cookie('show_total_hashrate'); // Might make this a ftmgc setting instead of cookie
...
function showToastUpdate(currentVersion, newestVersion) {
  ...
  $.cookie('ftmgc_version', newestVersion, { expires: 1 });
}
function showToastNoUpdate(currentVersion) {
  $().toastmessage('showToast', {
    sticky  : true,
    text    : '<b>No Update Available</b><br />You are currently running the latest build (<b class="current">'+currentVersion+'</b>) of ftmgc.',
    type    : 'notice'
  });
}
...
function showToastRigOffline(name) {
  $().toastmessage('showToast', {
    sticky  : true,
    text    : 'ftmgc cannot connect to the rig "' + name + '".<br />This probably means the rig is offline.',
    type    : 'warning'
  });
}
