...
class ftmgc {
    ...
    public function saveSettings($data) {
        $fh = $fileHandler = new FileHandler('configs/ftmgc.json');
        ...
        $this->_config['ftmgc'] = $settings;

        if ($fh->write(json_encode($settings)) !== false) {
            if (isset($_COOKIE['ftmgc_version'])) {
                unset($_COOKIE['ftmgc_version']);
                setcookie('ftmgc_version', null, -1, '/');
            }
