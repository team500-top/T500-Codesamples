...
?>
    ...
    <?php if (count($ftmgc->getMiners()) == 0 && count($ftmgc->getPools()) == 0 && count($ftmgc->getWallets()) == 0) { ?>
   ...
   <div id="dashboard-wrap" class="container sub-nav">
    ...
    ?>
      ...
      <?php
      foreach ($ftmgc->getPools() as $poolId => $pool) {
        $poolId++;
      ...
      <?php
      if (count($ftmgc->getWallets()) > 0) {
        include("templates/panels/wallet.php");
      }

      if (count($ftmgc->getMiners()) > 0 || count($ftmgc->getPools()) > 0) {
