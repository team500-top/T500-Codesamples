...
if (isset($_POST['ftmgc_version']) &&
    ($_POST['ftmgc_version'] != CURRENT_VERSION) &&
    ...
    if ($settings['general']['updates']['enabled'] == 1) {
        ...
        if (!mkdir($currentDir . $updateDir, 0777, true) && !file_exists($currentDir . $updateDir)) {
            ...
            echo '********************' . '<br />';
            echo 'There is a problem with the user permissions that will not allow ftmgc to update.' . '<br />';
            if ($osType == 'linux') {
                echo 'Please run the command "sudo sh permissionfix.sh" in the root of the ftmgc directory to fix this issue.' . '<br />';
            }
            echo '********************' . '<br />';
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, 'Mozilla/4.0 (compatible; ftmgc ' . CURRENT_VERSION . '; PHP/' . phpversion() . ')');
        ...
        echo '----------<br />';
        echo '==> Updating ftmgc...<br />'; ob_flush(); flush(); sleep(1);
        ...
        echo '----------<br />';
        echo '** ftmgc was updated successfully! **';
              ...
              <div class="stat-pair">
                <div class="stat-label">
                  <form method="POST" target="cg_update">
                    ...
                    <p>The version of ftmgc that you're running <span class="orange">(<?php echo CURRENT_VERSION ?>)</span> can be updated to <span class="green"><?php echo $_COOKIE['ftmgc_version'] ?></span>.</p>
                    ...
                    <input type="hidden" name="ftmgc_version" value="<?php echo $_COOKIE['ftmgc_version'] ?>" />
