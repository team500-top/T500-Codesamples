...
class WPSEODreamMeta {
	...
	public function get_meta_title($title){
		...
		global $post;
		$post_specific_title = get_post_meta($post->ID, 'WPSEODream_title', TRUE);
		
		if (!empty($post_specific_title)) {
			return $post_specific_title;
		}
		
		$use_automatic_titles = get_option('WPSEODream_use_automatic_titles') === WPSEODream::OPTION_ON;
		
		if (!$use_automatic_titles) {
			return $title;
		}
		
		$type_specific_title = get_option('WPSEODream_automatic_title_'.$post->post_type);
	...
	private function get_meta_description_unencoded() {
		$front_page_description_option = get_option('WPSEODream_frontpage_description');
	...
	public function print_meta_opengraph() {
		...
		$facebook_app_id = get_option('WPSEODream_facebook_app_id', FALSE);
	...
	public function print_meta_noindex() {
		...
		if (is_tag()) {
			$noindex_for_tags = get_option('WPSEODream_noindex_for_tags');
			if ($noindex_for_tags === WPSEODream::OPTION_ON) {
				$print_noindex = TRUE;
			}
		}
		
		if (is_category()) {
			$noindex_for_categories = get_option('WPSEODream_noindex_for_categories');
			if ($noindex_for_categories === WPSEODream::OPTION_ON) {
				$print_noindex = TRUE;
			}			
		}
		
		if (is_date() || is_author()) {
			$noindex_for_archives = get_option('WPSEODream_noindex_for_archives');
			if ($noindex_for_archives === WPSEODream::OPTION_ON) {
				$print_noindex = TRUE;
			}				
		}
		
		if (is_paged()) {
			$noindex_for_paged_pages = get_option('WPSEODream_noindex_for_paged');
			if ($noindex_for_paged_pages === WPSEODream::OPTION_ON) {
				$print_noindex = TRUE;
			}			
		}
	...
	public function print_script_version() {
		echo "<!-- Open WordPress SEO " . WPSEODream::VERSION . " -->\n";
	}
	
	public function print_google_analytics_script() {
		$tracking_code = get_option('WPSEODream_google_analytics_code', FALSE);
