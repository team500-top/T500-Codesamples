...
class WPSEODreamSitemap {
	...
	public function __construct() {
		$this->imageSitemap = new WPSEODreamImageSitemap();
		$this->pinger = new WPSEODreamPing();
	}
	
	public function create_sitemap_from_control_panel() {
		$sitemap_enabled = get_option('WPSEODream_sitemap_enabled') === WPSEODream::OPTION_ON;
		
		if (!$sitemap_enabled) {
			update_option('WPSEODream_sitemap_create_success', 'not_enabled');
		...
		if ($sitemap_success) {
			update_option('WPSEODream_sitemap_create_time', time());
			update_option('WPSEODream_sitemap_create_success', WPSEODream::STATUS_OK);
			$this->pinger->ping_search_engines();
		}
		else {
			update_option('WPSEODream_sitemap_create_success', WPSEODream::STATUS_ERROR);
		}
		
		if ($image_sitemap_success) {
			update_option('WPSEODream_image_sitemap_create_time', time());
			update_option('WPSEODream_image_sitemap_create_success', WPSEODream::STATUS_OK);
		}
		else {
			update_option('WPSEODream_image_sitemap_create_success', WPSEODream::STATUS_ERROR);
		}
	...
	private function generate_sitemap_xml() {
		$option_priorities = get_option('WPSEODream_sitemap_prioritities');
		$option_include_lastmod = get_option('WPSEODream_sitemap_include_lastmod') === WPSEODream::OPTION_ON;
		$option_include_tags = get_option('WPSEODream_sitemap_include_tags') === WPSEODream::OPTION_ON;
		$option_include_categories = get_option('WPSEODream_sitemap_include_categories') === WPSEODream::OPTION_ON;
